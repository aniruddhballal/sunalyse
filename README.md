...
Sunalyse lets you navigate through 20 years of solar magnetic field data and explore three layers of solar physics:
...
Approximate timing per CR: ~100s for Br grid + ~270s for field line tracing = ~6 minutes total.

---

## What We Built from 13th of March, 2026 onwards (project started out early December 2025)
...
### Near term
- **Neutral line enhancement** — currently the polarity surface shows the HCS as a colour boundary. Rendering an explicit bright curve at Br=0 on the surface would make the current sheet position more precise.
- **Runge-Kutta integration** — replacing Euler with RK4 would halve computation time while improving field line accuracy.
- **Field line density control** — UI slider for number of seeds (currently fixed at 500).

### Longer term
- **Solar cycle animation** — animate through all 181 CRs showing the HCS tilt angle evolving through the solar cycle (flat ~2° at solar minimum, warped ~60°+ at solar maximum).
- **Synchronic maps** — replace synoptic maps with synchronic maps to eliminate the east-west seam discontinuity.
- **Higher lmax** — increasing from lmax=85 to lmax=120+ would resolve finer active region structure in the field lines, at the cost of longer alm computation.
- **Open/closed field boundary** — highlight the boundary between open (connecting to solar wind) and closed (returning to photosphere) field regions on the photosphere itself.
- **Spacecraft trajectory overlay** — show the position of spacecraft like Parker Solar Probe or Solar Orbiter against the coronal field structure.
